...
export default function ProtectedRoute({ children, conditionFn = (user) => (user?.authenticated), to = '/login' }: { children: ReactNode, conditionFn?: (user: UserSelfData) => boolean, to?: To }) {
    const { user } = useUser();
